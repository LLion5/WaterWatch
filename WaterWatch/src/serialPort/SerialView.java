...
import cn.zjgsu.waterwatch.util.PortToData;
    ...
    class SerialListener implements SerialPortEventListener {
        ...
        public void serialEvent(SerialPortEvent serialPortEvent) {
            ...
            case SerialPortEvent.DATA_AVAILABLE: // 1 串口存在可用数据
                try {
                	 //System.out.println("串口有数据");
                	 SerialView.this.str.append(SerialTool.readPhAndTurbidity(SerialView.this.getSerialPort()));
             			if(SerialView.str.length()>=1) {
             				System.out.println(SerialView.str.toString());
             				String time=df.format(LocalDateTime.ofInstant(Instant.ofEpochMilli(System.currentTimeMillis()),ZoneId.of("Asia/Shanghai")));
             				receiveArea.append(time+" ["+serialPort.getName().split("/")[3]+"] : "+ str+"\r\n");
                            receiveArea.setCaretPosition(receiveArea.getText().length());  
                            
                            String PH = SerialView.this.str.substring(SerialView.this.str.indexOf("PH:")+3,SerialView.this.str.indexOf("PH:")+6);
                            String Turbidity = SerialView.this.str.substring(SerialView.this.str.indexOf("Turbidity")+10);
                            System.out.println(PH+Turbidity);
                            
                            new PortToData(PH + ";" + Turbidity + ";" + "1");
